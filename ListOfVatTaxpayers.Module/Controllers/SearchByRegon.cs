...
namespace ListOfVatTaxpayers.Module.Controllers
{
    public partial class SearchByRegon : ViewController
    {
        ...
        async void Action_SearchByRegon(object sender, SimpleActionExecuteEventArgs e)
        {
            Session currentSession = ((XPObjectSpace)this.ObjectSpace).Session;

            var currentObj = e.CurrentObject as EntityView;

            Tuple<string, bool> result = await _apiConnection.InvokeEndpointAsync($"/api/search/regon/{currentObj.Field}?date={currentObj.Data}");
